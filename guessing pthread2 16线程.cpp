...
void PriorityQueue::GenerateParallel(PT pt) {
    ...
    if (pt.content.size() == 1) {
        ...
        int num_threads = std::min(16, std::max(1, work_size / 5000));
        ...
        for (int t = 0; t < num_threads; t++) {
            ...
            auto task = [this, a, start, end, &result_mutex]() {
                ...
                for (int i = start; i < end; i++) {
                    string guess = a->ordered_values[i];
                    local_results.emplace_back(std::move(guess));
                }
        ...
        for (int t = 0; t < num_threads; t++) {
            ...
            auto task = [this, a, start, end, guess, &result_mutex]() {
                ...
                for (int i = start; i < end; i++) {
                    string temp = guess + a->ordered_values[i];
                    local_results.emplace_back(std::move(temp));
                }
