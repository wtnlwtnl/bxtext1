...
Args parse_args(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (key == "--input") {
            ...
            args.output = value;
        } else if (key == "--full-output") {
            args.full_output = value;
        } else if (key == "--alpha") {
...
PageRankResult pagerank(
    ...
    for (int iter = 1; iter <= max_iter; ++iter) {
        ...
        for (size_t block = 0; block < block_graph.block_starts.size(); ++block) {
            const uint32_t start = block_graph.block_starts[block];
            const uint32_t end = block_graph.block_ends[block];
            const uint32_t base = block_graph.block_bases[block];
...
void write_full_scores(
    ...
    std::ofstream output(output_path);
    if (!output) {
        throw std::runtime_error("Failed to open full output file: " + output_path);
    }
    output << std::fixed << std::setprecision(15);
    for (size_t i = 0; i < raw_node_ids.size(); ++i) {
        output << raw_node_ids[i] << ' ' << rank[i] << '\n';
    }
}
...
int main(int argc, char** argv) {
    try {
        ...
        if (!args.full_output.empty()) {
            write_full_scores(args.full_output, graph.raw_node_ids, result.rank);
        }
