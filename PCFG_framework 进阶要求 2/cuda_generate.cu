...
#include <mutex>
#include <omp.h>
#include <cstring>  // 添加这个头文件用于strcpy
#include <thread>
...
#define USE_TEXTURE_MEMORY 0  // 设置为0避免弃用警告
...
// 任务池资源
static std::vector<cudaStream_t> g_taskStreams;
static const int MAX_TASK_STREAMS = 8;
static std::mutex taskStreamMutex;
...
void initGPUResources() {
    ...
    // 创建异步任务流
    g_taskStreams.resize(MAX_TASK_STREAMS);
    for (int i = 0; i < MAX_TASK_STREAMS; i++) {
        CUDA_CHECK(cudaStreamCreate(&g_taskStreams[i]));
    }
...
// 获取可用的任务流
cudaStream_t getAvailableTaskStream() {
    std::lock_guard<std::mutex> lock(taskStreamMutex);
    static int nextStreamIndex = 0;
    
    cudaStream_t stream = g_taskStreams[nextStreamIndex];
    nextStreamIndex = (nextStreamIndex + 1) % MAX_TASK_STREAMS;
...
void cacheSegmentValues(int type, int length, const std::vector<std::string>& values) {
    ...
    // 填充主机内存
    for (size_t i = 0; i < values.size(); i++) {
        strcpy(&h_values[valueOffsets[i]], values[i].c_str());
    }
